...
//NEW// Keyframes
float posXavion = 2.0, posYavion = -1.0, posZavion = -3.0;
float	movAvion_x = 0.0f, movAvion_y = 0.0f;
float giroAvion = 0;

#define MAX_FRAMES 100 //Número de cuadros máximos
int i_max_steps = 400; //Número de pasos entre cuadros para interpolación, a mayor número , más lento será el movimiento
int i_curr_steps = 0;
...
void saveKeyFramesToFile() {
	...
	file.close();
	std::cout << "\n Keyframes guardados en keyframes.txt\n";
}
...
void saveFrame(void)
{
	...
	//Se agregan nuevas líneas para guardar más variables
	FrameIndex++;
	saveKeyFramesToFile();
}
...
int main()
{
	...
	KeyFrame[1].movAvion_x = -4.0f;
	KeyFrame[1].movAvion_y = 0.0f;
	KeyFrame[1].giroAvion = 0;


	KeyFrame[2].movAvion_x = -8.0f;
	KeyFrame[2].movAvion_y = 4.0f;
	KeyFrame[2].giroAvion = 0;


	KeyFrame[3].movAvion_x = -12.0f;
	KeyFrame[3].movAvion_y = 8.0f;
	KeyFrame[3].giroAvion = 0;


	KeyFrame[4].movAvion_x = -16.0f;
	KeyFrame[4].movAvion_y = 12.0f;
	KeyFrame[4].giroAvion = 0.0f;

	KeyFrame[5].movAvion_x = -20.0f;
	KeyFrame[5].movAvion_y = 12.0f;
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_9])
	{
		if (ciclo < 1)
		{
			...
			ciclo++;
			ciclo2 = 0;
			printf("\n Presiona la tecla G para poder habilitar la variable\n");
		}
