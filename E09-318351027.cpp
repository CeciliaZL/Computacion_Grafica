...
float rotAlaOffset = 9.0f;     // velocidad alas
bool aletea = true;             //Roota ala
//Cuerpo
float avanceDragon = 0.0f;
float movDragonOffset = 0.1f;   // velocidad dragon
bool avanzaDragon = true;       // Dragon hacia adelnate o tras
float limiteAvanceDragon = 20.0f;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		*/
		...
		if (aletea)
		{
			if (anguloAla < 30.0f)
			{
				anguloAla += rotAlaOffset * deltaTime;
			}
			else
			{
				aletea = !aletea; // cambia dirección
			}
		}
		else
		{
		...
		if (avanzaDragon)
		{
			if (avanceDragon < limiteAvanceDragon)
			{
				avanceDragon += movDragonOffset * deltaTime;
			}
			else
			{
		...
		//Dragon sin alas
		/*model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 10.5f, 0.266f));
		...
		//AlaIzq
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 11.0f, 0.266f ));
		modelaux = model;
		model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		// Dragon sin alas
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(avanceDragon, 8.0f + sin(glm::radians(angulovaria)) * 2.0f, 0.0f));

		// Volteacuerpo si llega al limite
		if (avanzaDragon)
		{
		...
		// AlaDer 
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.5f, 1.5f, -0.8f));
		model = glm::rotate(model, glm::radians(anguloAla), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		alaDer.RenderModel();
		...
		*/
		...
		// Cambio de textura
		tiempoAcumulado += deltaTime * 0.5f; 
		if (tiempoAcumulado <= 25.0f)
		{
			Numero1Texture.UseTexture(); // muestra número 1
		}
		else
		{
			Numero2Texture.UseTexture(); // muestra número 2
			if (tiempoAcumulado >= 50.0f) // reinicia ciclo
			{
				tiempoAcumulado = 0.0f;
			}
		}
